...
defined('BASEPATH') OR exit('No direct script access allowed');
        ...
        public function index()
        {
            ...
            //memmanggil fungsi
            $output['UnitCost_Berdasarkan_Region']=$this->UnitCost_Berdasarkan_Region($data);
        ...
        function UnitCost_Berdasarkan_Region($data)
        {
            $result=array();
            foreach ($data as $row) {
                if(isset($result[$row->Region]) == false) 
                ...
                {
                    $result[$row->Region]=$row->UnitCost;
                }else {
                    $UnitCost=$result[$row->Region];
                    $result[$row->Region]=$UnitCost + (int)$row->UnitCost;
                }
            };
            // konversi dalam format tabulasi

            $keys=array_keys($result);
            $tabs=[['Region','UnitCost']];
            foreach($keys as $row)
            {
                $dt=[$row,$result[$row]];
                array_push($tabs,$dt);
